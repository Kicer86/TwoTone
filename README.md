...
Working Directory: by default temporary files live in a per-run subdirectory of the operating system's user data location for the application, cleaned up automatically. Use -w or --working-dir to point at a custom directory - it is then used exactly as given (no per-run subdirectory) and removed whole when the run finishes. Because the whole directory is deleted on exit, twotone refuses to start if the given directory already exists and is not empty, so a run never wipes pre-existing data. With --keep-wd nothing is deleted from the working directory (and this non-empty check is lifted), which preserves all intermediate files (frames, audio segments, debug dumps) for inspection.<br/>
