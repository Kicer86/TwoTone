...
Working Directory: by default temporary files live in a per-run subdirectory of the operating system's user data location for the application; leftovers of crashed runs are removed automatically at startup. Use -w or --working-dir to point at a custom directory - it is then used exactly as given (no per-run subdirectory) and twotone never scans or removes other data there. With --keep-wd nothing is deleted from the working directory, which preserves all intermediate files (frames, audio segments, debug dumps) for inspection.<br/>
