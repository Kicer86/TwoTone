...
from twotone.tools.utils2 import generic, process, subtitles as subs, video as vid


class Fixer(generic.InterruptibleProcess):
    def __init__(self, logger: logging.Logger, really_fix: bool):
        super().__init__()
        self.logger = logger
        self._do_fix = really_fix

    def _print_broken_videos(self, broken_videos_info: [(vid.VideoInfo, [int])]):
    ...
    def _no_resolver(self, video_track: vid.VideoTrack, content: str):
        self.logger.error("Cannot fix the file, no idea how to do it.")
        return None

    def _long_tail_resolver(self, video_track: vid.VideoTrack, content: str):
        timestamps = list(subs.subrip_time_pattern.finditer(content))
        last_timestamp = timestamps[-1]
        time_from, time_to = map(generic.time_to_ms, last_timestamp.groups())
    ...
    def _fps_scale_resolver(self, video_track: vid.VideoTrack, content: str):
        target_fps = generic.fps_str_to_float(video_track.fps)
        multiplier = subs.ffmpeg_default_fps / target_fps

        return subs.alter_subrip_subtitles_times(content, multiplier)
    ...
    def _fix_subtitle(self, broken_subtitle, video_info: vid.VideoInfo) -> bool:
    ...
    def _extract_all_subtitles(self,video_file: str, subtitles: [subs.Subtitle], wd: str) -> [subs.SubtitleFile]:
        result = []
        options = ["tracks", video_file]

        for subtitle in subtitles:
            outputfile = f"{wd}/{subtitle.tid}.srt"
            subtitleFile = subs.SubtitleFile(path=outputfile, language=subtitle.language, encoding="utf8")
    ...
    def _repair_videos(self, broken_videos_info: [(vid.VideoInfo, [int])]):
                ...
                with tempfile.TemporaryDirectory() as wd_dir:
                    ...
                    if status:
                        if self._do_fix:
                            ...
                            vid.generate_mkv(input_video=video_without_subtitles, output_path=temporaryVideoPath, subtitles=subtitles)
    ...
    def _check_if_broken(self, video_file: str): # -> (video.VideoInfo, [int]) | None:    // FIXME
        ...
        video_info = vid.get_video_data(video_file)
